...
= Задание
== Предметная область, для которой будет разрабатываться информационная система
\ Согласована предметная область: _система контроля кондиционеров_.

== Подробное текстовое описание предметной области

\ Система контроля кондиционеров предназначена для централизованного управления кондиционерами, установленными в различных зданиях и помещениях.
...
Система также интегрируется с температурными датчиками, установленными в помещениях. Эти датчики позволяют получать информацию о текущей температуре воздуха в реальном времени.
...
- *D01.* Для разработки бэкенда использовать Java, Spring Framework с необходимыми модулями.
...
- *D04.* Для связи бэкенда и фронтенда использовать протокол HTTP, спецификация REST.
